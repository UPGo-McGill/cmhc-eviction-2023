...
t_hc <- function(...) {
  transcripts |> 
    summarize(
      ...
      no_change = sum(code == "HCC-N") >= 1, .by = id) |> 
    group_by(...) |> 
...
t_eil <- function(...) {
  transcripts |> 
    summarize(
      ...
      positive = sum(code == "EIL-PI") >= 1, .by = id) |> 
    group_by(...) |> 
...
figure_4 <- 
  ...
  summarize(
    ...
    region = sum(code == "M-R") >= 1,
    prov = sum(code == "M-P") >= 1,
    country = sum(code == "M-out") >= 1, .by = id) |> 
...
# Loss of social life
transcripts |> 
  filter(sum(category == "M") >= 1, .by = id) |> 
  summarize(
    stayed = sum(code == "M-S") >= 1,
    social = sum(code == "EIL-SL") >= 1, .by = id) |> 
  group_by(stayed) |> 
